...
- ``$XONSH_WIN_DIR_PERMA_CACHE``: unchanging dirs like ``C:\Windows\System32\`` for permanent caching
- ``$XONSH_WIN_DIR_SESSION_CACHE``: rarely changing dirs for caching per Xonsh session
- ``$XONSH_WIN_PATH_DIRS_TO_LIST``: small dirs (a few dozen files) for full file listing
...
For smaller dirs (~few dozen files) it's faster to list the dir, so you can add such dirs to ``$XONSH_WIN_PATH_DIRS_TO_LIST`` to reduce typing lag

And to reduce the typing lag even more (at the cost of a small loss of precision of syntax highlighting) you can list all the rarely changing (like ``C:\Program Files\Python\``) or not very important dirs from your ``$PATH`` in ``$XONSH_WIN_DIR_SESSION_CACHE``
...
You can also cache large never changing dirs (like ``C:\Windows\System32\`` with thousands of files) permanently by adding them to ``$XONSH_WIN_DIR_PERMA_CACHE`` (stored in ``$XONSH_CACHE_DIR\win-dir-perma-cache.pickle``)
