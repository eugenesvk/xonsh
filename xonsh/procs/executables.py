...
class PathCache:  # Singleton
    ...
    def get_cache_info(self, v=0):
        ...
        save_list = "🖫" if env.get("XONSH_DIR_CACHE_LIST_SAVE_ON_EXIT", True) else "✗"
        c = ColorShort()
        msg = f"""\
        ...
        if v >= 1:
            msg += f"""
                                                   ...
                                                   ({        save_list.rjust(2)}                         $XONSH_DIR_CACHE_LIST_SAVE_ON_EXIT)\
        """
        msg += f"""
        ...
        if v >= 1:
            msg += (
                "\n"
                "Get cached file data: pc = PathCache with key = path, value = (mtime+) trie of files|commands (depending on …_NON_EXE))\n"
